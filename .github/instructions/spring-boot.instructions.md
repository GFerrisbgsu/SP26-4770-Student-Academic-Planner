...
- **⚠️ Spring Boot 4.0+**: `@AutoConfigureMockMvc` does not exist. See "Spring Boot 4.0 / Spring Security 7 Considerations" section below for manual MockMvc configuration pattern.
...
- **Spring Boot 4.0.2 removed web testing annotations**: `@WebMvcTest` and `@AutoConfigureMockMvc` **do not exist**.
- **MANDATORY**: Manually configure MockMvc in `@BeforeEach` method:
  ```java
  ...
  ```
- **ObjectMapper**: Cannot be autowired in test context - create manually with `new ObjectMapper()`.
- **For Spring Security 7**: Add `.apply(springSecurity())` before `.build()` if testing secured endpoints.
- Without manual configuration, MockMvc will be null and tests will fail with "No qualifying bean" errors.

### Removed Test Annotations (Spring Boot 4.0.2+)
The following annotations **do not exist** in Spring Boot 4.0.2 and will cause compilation errors:
- ❌ `@WebMvcTest` - Use `@SpringBootTest` with manual MockMvc configuration instead
- ❌ `@AutoConfigureMockMvc` - Use manual `MockMvcBuilders.webAppContextSetup()` in `@BeforeEach`
- ❌ Spring Boot 4.0.2 removed the `spring-boot-test-autoconfigure-web-servlet` package entirely
- **Migration**: All controller tests must use `@SpringBootTest` with manual MockMvc setup (see above pattern)
...
7. **Spring Boot 4.0 Testing**: `@WebMvcTest` and `@AutoConfigureMockMvc` don't exist - must manually configure MockMvc in `@BeforeEach`.
